...
class JournalUploadHandler(UploadHandler):
    ...
    def _get_other_ids(self, row, main_id):
        issn = str(row.get("Journal ISSN (print version)", "")).strip()
        eissn = str(row.get("Journal EISSN (online version)", "")).strip()
        others = [i for i in (issn, eissn) if i and i != main_id]
        return others
    ...
    def pushDataToDb(self, file_path):
        ...
        for _, row in df.iterrows():
            ...
            for other_id in self._get_other_ids(row, journal_id):
                add_triple(self.PRED['id'], Literal(other_id).n3())
...
class CategoryQueryHandler(QueryHandler):
    ...
    def _execute_query(self, query, params=None):
        ...
        return df
    def getById(self, id):
        by_category = self._execute_query("SELECT category_id, MAX(quartile) AS quartile, MAX(area) AS area FROM journal_category WHERE category_id=? GROUP BY category_id", (id,))
        if not by_category.empty:
            return by_category
        return self._execute_query("SELECT DISTINCT area FROM journal_category WHERE area=?", (id,))
    def getAllCategories(self):
        return self._execute_query("SELECT category_id, MAX(quartile) AS quartile, MAX(area) AS area FROM journal_category GROUP BY category_id")
    ...
    def getCategoriesWithQuartile(self, quartiles: set):
        ...
        return self._execute_query(f"SELECT category_id, MAX(quartile) AS quartile, MAX(area) AS area FROM journal_category WHERE quartile IN ({placeholders}) GROUP BY category_id", tuple(quartiles))
    def getCategoriesAssignedToAreas(self, area_ids: set):
        if not area_ids:
            return self.getAllCategories()
        placeholders = ",".join("?" for _ in area_ids)
        return self._execute_query(f"SELECT category_id, MAX(quartile) AS quartile, MAX(area) AS area FROM journal_category WHERE area IN ({placeholders}) GROUP BY category_id", tuple(area_ids))
    ...
    def getAreasByName(self, area_partial_name):
        return self._execute_query("SELECT DISTINCT area FROM journal_category WHERE area LIKE ?", ('%' + area_partial_name + '%',))
...
class JournalQueryHandler(QueryHandler):
    ...
    def getJournalsWithLicense(self, licenses):
        wanted = {licenses} if isinstance(licenses, str) else set(licenses or [])
        ...
        def has_match(cell_value):
            parts = {p.strip() for p in str(cell_value).split(',')}
            return len(parts & wanted) > 0
        good_subjects = set(lic_rows[lic_rows['object'].apply(has_match)]['subject'])
        return df[df['subject'].isin(good_subjects)].reset_index(drop=True)
    ...
    def getJournalsWithDOAJSeal(self):
        ...
        return self._query_to_df(q)
    def getJournalsWithSealAndNoAPC(self):
        q = f'SELECT ?s ?p ?o WHERE {{ ?s <{self.base_uri}seal> true ; <{self.base_uri}apc> false . ?s ?p ?o . }}'
        return self._query_to_df(q)
...
class FullQueryEngine(BasicQueryEngine):
    def _journal_id_pairs(self, journal_df_long, subject_col='subject'):
        pred_name = journal_df_long['predicate'].apply(lambda p: str(p).split('/')[-1].split('#')[-1])
        id_only = journal_df_long[pred_name == 'id']
        return id_only[[subject_col, 'object']].rename(columns={subject_col: 'uri', 'object': 'issn'})
    ...
    def getJournalsInCategoriesWithQuartile(self, category_ids: set, quartiles: set):
        ...
        id_pairs = self._journal_id_pairs(journal_df_long)

        category_links_df = self._get_combined_df(self.categoryQuery, 'getCategoryLinks')
        if category_links_df.empty: return []

        matching_links = category_links_df
        if category_ids:
            matching_links = matching_links[matching_links['category_id'].isin(category_ids)]
        if quartiles:
            matching_links = matching_links[matching_links['quartile'].isin(quartiles)]

        good_uris = set(pd.merge(id_pairs, matching_links, on='issn')['uri'])
        merged_df = journals_df_wide[journals_df_wide['uri'].isin(good_uris)]
    ...
    def getJournalsInAreasWithLicense(self, area_ids: set, licenses: set):
        ...
        id_pairs = self._journal_id_pairs(journal_df_long)
        category_links_df = self._get_combined_df(self.categoryQuery, 'getCategoryLinks')
        matching_links = category_links_df[category_links_df['area'].isin(area_ids)]
    ...
    def getDiamondJournalsInAreasAndCategoriesWithQuartile(self, area_ids: set, category_ids: set, quartiles: set):
        ...
        matching_links = category_links_df
        if area_ids:
            matching_links = matching_links[matching_links['area'].isin(area_ids)]
        if category_ids:
            matching_links = matching_links[matching_links['category_id'].isin(category_ids)]
        if quartiles:
            matching_links = matching_links[matching_links['quartile'].isin(quartiles)]
        ...
        good_uris = set(pd.merge(id_pairs, matching_links, on='issn')['uri'])
        merged_df = diamond_journals_df[diamond_journals_df['uri'].isin(good_uris)]

        return self._wide_df_to_journals(merged_df)

    def getDiamondJournalsInAreas(self, area_partial_name):
        journal_df_long = self._get_combined_df(self.journalQuery, 'getJournalsWithSealAndNoAPC')
        if journal_df_long.empty: return []
        ...
        category_links_df = self._get_combined_df(self.categoryQuery, 'getCategoryLinks')
        if category_links_df.empty: return []
        matching_links = category_links_df[category_links_df['area'].str.contains(area_partial_name, case=False, na=False, regex=False)]
